...
# Relabel vital status and factorise hisological subtype for easier downstream processing
clin.liu.ord$vital_status <- ifelse(clin.liu.ord$vital_status == "Dead", 1, 0)

# Factorise histological subtypes          
hist.fac <- factor(as.vector(as.character(exp.info.ord[4,])),
                   levels = c("DDLPS", "UPS", "STLMS", "ULMS", "MPNST", "MFS", "SS"),
                   labels = c(1, 2, 3, 4, 5, 6, 7)
)

exp.info.ord <- rbind(exp.info.ord, hist.fac) # Factors on row 6
...
for (i in 1:ncol(resample.ind)) {
  ...
  for (point in cutpoints){
    ...
    surv.inf <- data.frame(
      ...
      exp.goi = exp.split,
      histype = as.vector(as.character(exp.info.ord.res[4,]))
    )
    
    # Fit survival objects
    surv.obj <- Surv(time = as.numeric(surv.inf$os.times), event = as.numeric(surv.inf$os.cen))
    surv.fit <- survfit(surv.obj ~ exp.goi + histype, data = surv.inf)
...
ci.cuts <- ggplot() + 
  ...
  labs(title = paste("Distribution of", n.resample, "bootstrap cutoff for DSS"))
ggsave(file = paste(runpath, "/ds_95ci_cuts.png", sep=""), 
       plot = print(ci.cuts),
...
chiplot <- ggplot(data = chisqs.frame) +
  ...
  labs(title = paste0(n.resample, "-fold resampled chi-squared values for survival comparison at varying high/low cutoff for DSS")) +
  xlab("Transformed cutoff value") +
  ylab("Chi-squared") +
  theme(text = element_text(size=15))+
  guides(color = FALSE)

ggsave(file = paste(runpath, "/dss_chi_lines.png", sep=""), 
       plot = print(chiplot),
       width = 12, height = 8, units = "in",device = "png")
...
sel.exp.cutoff <- ggplot() +
  ...
  ylab("Patient count") +
  labs(fill = paste(goi, "expression level"), title = paste0("Selected ", goi, " expression cutoff for DSS"))

ggsave(file = paste0(runpath,"/dss_val_sel_goi_median_cutoff.png"),
...
surv.inf.val <- data.frame(
  os.cen = as.vector(as.character(clin.liu.ord.val$vital_status)),
  os.times = as.vector(as.numeric(clin.liu.ord.val$os.times)),
  exp.goi = opt.cut.split,
  histype = as.vector(as.character(exp.info.ord.val[4,])),
...
surv.fit <- survfit(surv.obj ~ exp.goi + histype, data = surv.inf.val)
...
#ggsave(file = paste0(runpath,"/dss_val_survival_split.png"),
       plot = print(ggsurv),
       width = 12, height = 8, units = "in",device = "png")
